...
class FormValidation {
  ...
  showFieldErrorMessages(error: FormError) {
    let msgContainer = error.element.parentElement.querySelector(
      "." + this.options.errorContainerClassName
    );
    if (msgContainer) {
      // reuse existing container
      let errorMsgContainers = msgContainer.querySelectorAll(
  ...
  validateForm() {
    const fields = this.form.querySelectorAll<HTMLFormValidatableElement>(
