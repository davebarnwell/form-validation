...
class FormValidation {
    ...
    showFieldErrorMessages(error) {
        ...
        for (let i = 0; i < error.messages.length; i++) {
            ...
            if (i === 0)
                error.element.focus();
            if (this.options.showFirstErrorOnly)
                break;
        }
    }
    removeFieldErrorMessages(element) {
    ...
    validateField(field) {
        ...
        switch (field.tagName.toLowerCase()) {
            case 'input':
                error = this.validateInput(field);
                break;
            case 'textarea':
                error = this.validateTextarea(field);
                break;
            case 'select':
                error = this.validateSelect(field);
                break;
            default:
                break;
        }
        if (error) {
            ...
            this.removeFieldErrorMessages(event.target);
        }
        return error;
    }
    validateInput(input) {
